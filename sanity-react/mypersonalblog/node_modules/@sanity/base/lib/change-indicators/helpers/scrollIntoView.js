"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = void 0;

var _isScrollContainer = _interopRequireDefault(require("./isScrollContainer"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var SCROLL_INTO_VIEW_TOP_PADDING = -15;

var scrollIntoView = field => {
  var element = field.element;
  /*
   * Start at current element and check the parent for a scroll
   * bar until a scrollable element has been found.
   */

  var parentElementWithScroll = element;

  while (!(0, _isScrollContainer.default)(parentElementWithScroll)) {
    parentElementWithScroll = parentElementWithScroll.parentElement;
    /*
     * If the parent element is null it means we are at the root
     * element, which has no parent. Since no scroll bar has
     * been found so far it does not make sense to scroll.
     */

    if (!parentElementWithScroll) {
      return;
    }
  }

  parentElementWithScroll.scroll({
    top: parentElementWithScroll.scrollTop + field.rect.top - field.rect.bounds.top + SCROLL_INTO_VIEW_TOP_PADDING,
    left: 0,
    behavior: 'smooth'
  });
};

var _default = scrollIntoView;
exports.default = _default;