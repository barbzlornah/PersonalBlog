...
var _minimal = _interopRequireDefault(require("@sanity/observable/minimal"));
...
var progress = url => {
  return new _minimal.default(observer => {
...
var _default = {
  ...
  action: function () {
    var _action = _asyncToGenerator(function* (args, context) {
      ...
      var _yield$client$dataset = yield client.datasets.list().then(datasets => datasets.map(ds => ds.name)),
          _yield$client$dataset2 = _slicedToArray(_yield$client$dataset, 1),
          existingDatasets = _yield$client$dataset2[0];
      ...
      if (!existingDatasets.includes(sourceDatasetName)) {
        throw new Error("Dataset \"".concat(sourceDatasetName, "\" doesn't exist"));
      }
      ...
      if (existingDatasets.includes(targetDatasetName)) {
        throw new Error("Dataset \"".concat(targetDatasetName, "\" already exists"));
      }
      ...
      try {
        ...
        var listenUrl = client.getUrl("jobs/".concat(response.jobId, "/listen"));
        (0, _debug.default)("Listening on ".concat(listenUrl));
        progress(listenUrl).subscribe({
          next: event => {
            spinner.text = "~".concat(event.progress, "% done.");
          },
          error: () => {
            spinner.fail('There was an error copying the dataset.');
          },
          complete: () => {
            spinner.succeed("Finished copying dataset ".concat(_chalk.default.green(sourceDatasetName), " to ").concat(_chalk.default.green(targetDatasetName), "."));
          }
        });
      } catch (error) {
        if (error.statusCode) {
